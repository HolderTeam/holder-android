 ...
 * (1 Slide, 2 Straight, 3 Stack, 4 Swing, 5 Stealth, ..., 11 Spin), with [SURPRISE] -- not in
 * the brief -- last, so a future picker reads in the same order the brief does. */
enum class HolderCardSwipeStyle(val label: String, val description: String) {
    ...
    SURPRISE("Surprise", "A different style every swipe; the calm ones often, the wild ones rarely."),
}

/** Where a [ResolvedSwipeStyle.Swing] hinges. [LEADING_CORNER] follows the swipe -- bottom-left
 * when heading to the next card, bottom-right when heading back -- so the card always opens the
 * way the finger is going. */
enum class SwingPivot { BASE_CENTRE, LEADING_CORNER }

/** A [HolderCardSwipeStyle] with its actual numbers filled in. The user-facing setting is the
 * enum; this is what the renderer in CardViewPagerScreen dispatches on. For every setting except
...
sealed interface ResolvedSwipeStyle {
    ...
    /** The neighbour waits underneath at full size/opacity; the front card slides straight off
     * it (its natural drag-follow), so the neighbour reads as having been there all along
     * rather than sliding in from the edge. */
    data object Straight : ResolvedSwipeStyle

    /** Straight's placement, plus a small tilt on the departing front card about its centre --
     * the physical-pile illusion. A ~10-degree version read as far too much during prototyping;
     * the fixed [HolderCardSwipeStyle.STACK] uses 3. */
    data class Stack(val tiltDegrees: Float) : ResolvedSwipeStyle

    /** No spatial motion at all: every page cancels the pager's side-by-side placement and
     * stays put; the drag drives alpha alone, the front fading out as the neighbour fades in. */
    data object Stealth : ResolvedSwipeStyle

    /** The front card is pinned in place and rotates about a bottom pivot, so it reads as
     * hinged at its base and swinging aside to uncover the card beneath. Rotation follows the
     * finger. Neighbour waits underneath as in [Straight]. */
    data class Swing(val pivot: SwingPivot, val arcDegrees: Float) : ResolvedSwipeStyle

    /** The departing front card keeps its natural drag-follow and the neighbour slides in
     * normally (both so the gesture stays connected to the pager); on top of that the outgoing
     * card turns [degrees] on its way off. [followsFinger] picks the direction of the turn. */
    data class Spin(val degrees: Float, val followsFinger: Boolean) : ResolvedSwipeStyle
}

/** SURPRISE's grab-bag: a fixed set of pre-tuned variants, each drawn with the given relative
 * weight (roughly: 5 = common, 2 = uncommon, 1 = rare). By family this lands near Stack ~50% /
 * Swing ~33% / Spin ~17%, so most swipes are calm and the big turns are occasional. Slide and
 * Straight are left out (nothing to vary, and Slide is the anti-surprise); Stealth is left out
 * (its personality is having none). Swing never randomises its direction -- always toward the
 * finger; only Spin varies clockwise/anticlockwise. */
private val SURPRISE_BAG: List<Pair<ResolvedSwipeStyle, Int>> = listOf(
...
/** The set of variants SURPRISE can ever produce -- exposed for tests, and as the single place
 * that says "these nine and no others". */
...
fun HolderCardSwipeStyle.resolve(random: Random): ResolvedSwipeStyle = when (this) {
    ...
    HolderCardSwipeStyle.SWING -> ResolvedSwipeStyle.Swing(SwingPivot.BASE_CENTRE, arcDegrees = 70f)
    HolderCardSwipeStyle.STEALTH -> ResolvedSwipeStyle.Stealth
    HolderCardSwipeStyle.SPIN -> ResolvedSwipeStyle.Spin(degrees = 360f, followsFinger = false)
