...
extern "C" JNIEXPORT jstring JNICALL
Java_team_holder_android_HolderNative_nativeCardQuery(
    JNIEnv* env,
    jobject /* thiz */,
    jlong context_handle,
    jstring project_id,
  ...
  UtfChars project_id_chars(env, project_id);
  UtfChars request_json_chars(env, request_json);
  if (project_id_chars.get() == nullptr || request_json_chars.get() == nullptr) {
    throw_runtime(env, "project_id and request_json must not be null");
    return nullptr;
  }

  char* json = nullptr;
  holder_error* error = nullptr;
  const int rc = holder_card_query_json(
      context, project_id_chars.get(), request_json_chars.get(), &json, &error
  );
  if (rc != HOLDER_OK) {
    throw_runtime(env, error_message(error));
    return nullptr;
...
extern "C" JNIEXPORT jstring JNICALL
Java_team_holder_android_HolderNative_nativeCardMove(
    JNIEnv* env,
    jobject /* thiz */,
    jlong context_handle,
    jstring project_id,
  ...
  if (project_id_chars.get() == nullptr || card_id_chars.get() == nullptr ||
      request_json_chars.get() == nullptr) {
    throw_runtime(env, "project_id, card_id and request_json must not be null");
    return nullptr;
  }

  char* json = nullptr;
  holder_error* error = nullptr;
  const int rc = holder_card_move_json(
      context,
      project_id_chars.get(),
      card_id_chars.get(),
      request_json_chars.get(),
      &json,
      &error
  );
  if (rc != HOLDER_OK) {
...
extern "C" JNIEXPORT jstring JNICALL
Java_team_holder_android_HolderNative_nativeGitSyncNow(
    JNIEnv* env,
    jobject /* thiz */,
    jlong context_handle,
    jstring project_id,
  ...
  char* json = nullptr;
  holder_error* error = nullptr;
  const int rc = holder_git_sync_now(
      context,
      project_id_chars.get(),
      branch_chars.get(),
      push == JNI_TRUE ? 1 : 0,
